...
export interface EditorCanvasOptions<TData = unknown> {
        ...
        ) => NewComponent<TData> | null | undefined
        renderComponent?: (
...
export class EditorCanvas<TData = unknown> extends UiComponent<HTMLDivElement> {
        ...
        public constructor(options?: EditorCanvasOptions<TData>) {
                ...
                this.components = (options?.initialComponents ?? [{ id: 1, x: 100, y: 50, width: 80, height: 40 }]).map((component) =>
        ...
        private setupEventHandlers(): void {
                ...
                this.canvasElement.addEventListener("mousemove", (event) => {
                        ...
                        if (
                                hovered &&
                                this.isPointInsideComponent(hovered.component, x, y) &&
                                !this.isNearComponentEdge(hovered.component, x, y)
                        ) {
                                hovered = null
                        }
                ...
                this.canvasElement.addEventListener("mouseup", (event) => {
                        ...
                        if (this.isSelecting) {
                                ...
                                this.selectedIds = this.components
                                        .filter((component) =>
                                                component.x < x2 &&
                                                component.x + component.width > x1 &&
                                                component.y < y2 &&
                                                component.y + component.height > y1
                                        )
        ...
        private drawSelectionRectangle(): void {
                ...
                this.ctx.strokeRect(
                        this.selectStartX,
                        this.selectStartY,
                        this.selectCurrentX - this.selectStartX,
                        this.selectCurrentY - this.selectStartY
                )
                this.ctx.restore()
        }
        ...
        private getEdgeAnchor(
                ...
                x: number,
                y: number,
                tolerance = this.edgeHitPadding / this.scale
        ): EdgeAnchor<TData> | null {
        ...
        private computeAnchor(
                ...
                x: number,
                y: number
        ): { edge: EdgeName; point: { x: number; y: number }; distance: number } {
                const distances = [
