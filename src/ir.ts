...
export type PlaneRef =
	| { type: "datum"; name: "XY" | "YZ" | "ZX" | string }
	| { type: "offsetFace"; of: SelectorRef; offset: number }
	| { type: "planeBy3Pts"; p0: Vec3; p1: Vec3; p2: Vec3 };
...
export type AnchorRef = `${SketchEntityId}:p0` |
	`${SketchEntityId}:p1` |
...
export interface HolePattern extends BaseFeature {
	...
	on: SelectorRef;                     // target face
	grid:
	| { rows: number; cols: number; pitchX: number; pitchY: number; origin?: Vec2 }
	| { radial: true; count: number; radius: number; startDeg?: number };
...
export type AxisRef =
	| { type: "sketchLine"; of: `${FeatureId}:${SketchEntityId}` }
	| { type: "edge"; of: SelectorRef }
