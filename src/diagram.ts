...
const entityAttributeDescription = "Enter one attribute per line using `name:type` syntax. Optionally append `!` for required fields or `#` for primary keys."
...
export class DiagramEditor extends UiComponent<HTMLDivElement> {
	...
	private addPaletteSection(title: string, shapes: FlowchartShape[]): void {
		...
		for (const shape of shapes) {
			...
			item.addEventListener("click", (event) => {
				...
				const position = this.editor.toWorldPoint(bounds.left + bounds.width / 2, bounds.top + bounds.height / 2)
	...
	private buildComponentDefinition(shape: FlowchartShape, position: { x: number; y: number }, id?: number): Omit<DiagramComponent, "id"> & Partial<Pick<DiagramComponent, "id">> {
	...
	private renderComponent(ctx: CanvasRenderingContext2D, component: DiagramComponent, state: { selected: boolean }): void {
	...
	private drawRoundedRect(ctx: CanvasRenderingContext2D, component: DiagramComponent, selected: boolean, text: string, radius: number) {
		ctx.save()
	...
	private drawPredefinedProcess(ctx: CanvasRenderingContext2D, component: DiagramComponent, selected: boolean, text: string) {
		ctx.save()
		const stroke = selected ? "#2563eb" : "#0f172a"
		ctx.fillStyle = "#ffffff"
		ctx.strokeStyle = stroke
	...
	private drawManualInput(ctx: CanvasRenderingContext2D, component: DiagramComponent, selected: boolean, text: string) {
		ctx.save()
		const stroke = selected ? "#2563eb" : "#0f172a"
		ctx.fillStyle = "#ffffff"
		ctx.strokeStyle = stroke
	...
	private drawDocument(ctx: CanvasRenderingContext2D, component: DiagramComponent, selected: boolean, text: string) {
		...
		ctx.quadraticCurveTo(component.x + component.width * 0.75, component.y + component.height, component.x + component.width / 2, component.y + component.height - waveHeight / 2)
		ctx.quadraticCurveTo(component.x + component.width * 0.25, component.y + component.height - waveHeight, component.x, component.y + component.height)
		ctx.closePath()
		ctx.fill()
		ctx.stroke()
		this.drawCenteredText(ctx, text, component, "#0f172a")
	...
	private drawDatabase(ctx: CanvasRenderingContext2D, component: DiagramComponent, selected: boolean, text: string) {
		...
		ctx.quadraticCurveTo(component.x + component.width, component.y, component.x + component.width, component.y + radiusY)
		ctx.lineTo(component.x + component.width, component.y + component.height - radiusY)
		ctx.quadraticCurveTo(component.x + component.width, component.y + component.height, component.x + component.width / 2, component.y + component.height)
		ctx.quadraticCurveTo(component.x, component.y + component.height, component.x, component.y + component.height - radiusY)
		ctx.closePath()
		ctx.fill()
		ctx.stroke()
		ctx.beginPath()
		ctx.ellipse(component.x + component.width / 2, component.y + radiusY, component.width / 2, radiusY, 0, 0, Math.PI * 2)
		ctx.stroke()
		this.drawCenteredText(ctx, text, component, "#0f172a")
		ctx.restore()
	}
	...
	private roundRectPath(
		...
		if (br) ctx.quadraticCurveTo(component.x + component.width, component.y + component.height, component.x + component.width - r, component.y + component.height)
	...
	private drawCenteredText(ctx: CanvasRenderingContext2D, text: string, component: DiagramComponent, color: string, fontSize = 16) {
		ctx.save()
		ctx.fillStyle = color
