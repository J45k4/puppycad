...
impl<'a> Evaluator<'a> {
    ...
    fn eval_expr(
        ...
        match &expr.kind {
            ...
            ExprKind::Vector(values) => Ok(Value::Vec3([
                self.eval_expr(&values[0], scope, current)?
                    .as_number()
                    .ok_or_else(|| {
                        self.type_error(expr.span, "expected number for vector x component")
                    })?,
                self.eval_expr(&values[1], scope, current)?
                    .as_number()
                    .ok_or_else(|| {
                        self.type_error(expr.span, "expected number for vector y component")
                    })?,
                self.eval_expr(&values[2], scope, current)?
                    .as_number()
                    .ok_or_else(|| {
                        self.type_error(expr.span, "expected number for vector z component")
                    })?,
            ])),
            ...
            ExprKind::Ident(name) => scope
                ...
                .or_else(|| {
                    self.feature_graph
                        .has_decl(name.as_str())
                        .then(|| Value::NodeRef(name.clone()))
                })
                .ok_or_else(|| self.unknown_identifier(expr.span, current, name)),
            ExprKind::Reference(segments) => {
                self.resolve_reference(segments, scope, expr.span, current)
            }
            ...
            ExprKind::Unary { op, expr } => {
                ...
                match op {
                    ...
                    UnaryOp::Not => Ok(value.as_bool().map(Value::Bool).ok_or_else(|| {
                        self.type_error(expr.span, "unary '!' expects a boolean")
                    })?),
                }
            }
            ExprKind::Binary { op, left, right } => {
                ...
                match op {
                    BinaryOp::Or => {
                        let lhs = left_value.as_bool().ok_or_else(|| {
                            self.type_error(expr.span, "logical '||' expects booleans")
                        })?;
                        let rhs = right_value.as_bool().ok_or_else(|| {
                            self.type_error(expr.span, "logical '||' expects booleans")
                        })?;
                    ...
                    BinaryOp::Lt => self.compare_numbers(
                        expr.span,
                        left_value.as_number(),
                        right_value.as_number(),
                        |a, b| a < b,
                        "comparison '<'",
                    ),
                    BinaryOp::Le => self.compare_numbers(
                        expr.span,
                        left_value.as_number(),
                        right_value.as_number(),
                        |a, b| a <= b,
                    ...
                    BinaryOp::Gt => self.compare_numbers(
                        expr.span,
                        left_value.as_number(),
                        right_value.as_number(),
                        |a, b| a > b,
                    ...
                    BinaryOp::Ge => self.compare_numbers(
                        expr.span,
                        left_value.as_number(),
                        right_value.as_number(),
                        |a, b| a >= b,
    ...
    fn resolve_reference(
        ...
        let first = segments
            .first()
            .ok_or_else(|| self.syntax(span, current, "empty reference"))?;
        if segments.len() == 1 {
            return scope
                .get(first)
                .cloned()
    ...
    fn eval_call(
        ...
        let values = args
            .iter()
            .map(|arg| self.eval_expr(arg, scope, current))
        ...
        match name {
            ...
            "clamp" => {
                ...
                let value = values[0]
                    .as_number()
                    .ok_or_else(|| self.type_error(span, "clamp expects number arguments"))?;
                let min = values[1]
                    .as_number()
                    .ok_or_else(|| self.type_error(span, "clamp expects number arguments"))?;
                let max = values[2]
            ...
            "deg" => self.unary_fn(span, name, values.first().cloned(), |value| {
                value.to_radians()
            }),
            ...
            "vec3" => {
                ...
                Ok(Value::Vec3([
                    values[0]
                        .as_number()
                        .ok_or_else(|| self.type_error(span, "vec3 expects numeric arguments"))?,
                    values[1]
                        .as_number()
                        .ok_or_else(|| self.type_error(span, "vec3 expects numeric arguments"))?,
                    values[2]
    ...
    fn arith(
        ...
        let left = left
            .as_number()
            .ok_or_else(|| self.type_error(span, label))?;
        let right = right
            .as_number()
            .ok_or_else(|| self.type_error(span, label))?;
