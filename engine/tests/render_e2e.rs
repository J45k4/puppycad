...
fn render_one_frame_from_puppybot_to_folder() {
	...
	let output = if let Some(engine_bin) = std::env::var("CARGO_BIN_EXE_engine").ok() {
		Command::new(engine_bin)
			...
			.arg(&puppybot)
			.output()
			.expect("failed to run engine render with installed binary")
	} else {
		...
		Command::new(cargo_bin)
			...
			.arg(&puppybot)
			.output()
			.expect("failed to run engine render via cargo")
	};
