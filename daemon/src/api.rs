...
use puppycad_core::{
    build_model_state, build_render_state, codegen,
    eval::Evaluator,
...
impl ApiRenderStateApp {
    ...
    fn load_render_state(
        ...
        let window_id = state
            .windows
            .insert(pge::Window::new().title("render").ui(gui_id));
...
impl pge::App<ApiRenderEvent> for ApiRenderStateApp {
    ...
    fn on_event(&mut self, event: ApiRenderEvent, state: &mut pge::State) {
        match event {
            ApiRenderEvent::LoadRenderState(render_state) => {
                self.load_render_state(render_state, state)
            }
...
async fn handle_request(req: Request<Body>, state: ApiState) -> Result<Response<Body>, ApiError> {
    ...
    match (req.method(), segments.as_slice()) {
        ...
        (&Method::GET, ["api", "sessions", session_id, "files"]) => {
            ...
            let path = require_query_param(req.uri().query(), "path").ok_or_else(|| {
                ApiError::BadRequest("path query parameter is required".to_string())
            })?;
...
async fn get_file(session: Session, path: String) -> Result<Response<Body>, ApiError> {
    ...
    Ok(json_response(
        StatusCode::OK,
        FileGetResponse { path, text, etag },
    ))
}
...
async fn set_file(session: Session, request: FileSetRequest) -> Result<Response<Body>, ApiError> {
    ...
    if let Some(parent) = resolved.parent() {
        std::fs::create_dir_all(parent).map_err(|err| {
            ApiError::Internal(format!("failed to create parent directories: {err}"))
        })?;
    }
    if resolved.exists() {
...
async fn patch_file(
    ...
    match kind.as_str() {
        ...
        "json_patch" | "ast_patch" => Err(ApiError::BadRequest(
...
async fn check_model(
    ...
    evaluator
        ...
        .err()
        .iter()
        .for_each(|error| diagnostics.push(to_diagnostic(error)));
...
async fn render_models(
    ...
    let entry = request
        .entry_file
        .unwrap_or_else(|| session.entry_file.clone());
    let resolved = resolve_session_file(&session, &entry)?;
    let source = std::fs::read_to_string(&resolved)
        .map_err(|err| ApiError::BadRequest(format!("failed to read entry file: {err}")))?;
    ...
    for camera in cameras {
        ...
        if let Some(parent) = out_path.parent() {
            std::fs::create_dir_all(parent).map_err(|err| {
                ApiError::Internal(format!("failed to create render directory: {err}"))
            })?;
        }
...
async fn export_model(
    ...
    if request.format.as_str() != "glb" {
        return Err(ApiError::BadRequest(
            "only glb export is currently supported".to_string(),
        ));
    }
    let entry = request
        .entry_file
    ...
    if let Some(parent) = out_path.parent() {
        std::fs::create_dir_all(parent).map_err(|err| {
            ApiError::Internal(format!("failed to create export directory: {err}"))
        })?;
    }
...
async fn parse_json<T: DeserializeOwned>(req: Request<Body>) -> Result<T, ApiError> {
    ...
    serde_json::from_slice::<T>(&bytes)
        .map_err(|err| ApiError::BadRequest(format!("invalid request JSON: {err}")))
}
...
fn build_runtime(source: &str) -> Result<BuiltRuntime, ApiError> {
    ...
    let model =
        build_model_state(&graph).map_err(|err| ApiError::BadRequest(err.message.clone()))?;
...
fn render_single_frame(
    ...
    let app_result = pge::run_with_event_sender(app, |sender| {
        let _ = sender.send(ApiRenderEvent::LoadRenderState(
            runtime.render_state.clone(),
        ));
...
fn resolve_render_cameras(raw: Option<Vec<Value>>) -> Result<Vec<ResolvedCamera>, ApiError> {
    ...
    for entry in requested {
        ...
        return Err(ApiError::BadRequest(
            "invalid camera definition".to_string(),
        ));
    }
...
fn resolve_session_file(session: &Session, requested: &str) -> Result<PathBuf, ApiError> {
    ...
    if path.is_absolute() {
        return Err(ApiError::BadRequest(
            "path must be relative to session root".to_string(),
        ));
    }
    if path
        .components()
...
fn json_response<T: Serialize>(status: StatusCode, body: T) -> Response<Body> {
    ...
    *response.status_mut() = status;
    response.headers_mut().insert(
        ...
        "application/json"
            .parse()
            .expect("valid application/json header value"),
    );
    response
}
...
fn add_request_id(mut response: Response<Body>, request_id: String) -> Response<Body> {
    response.headers_mut().insert(
        ...
        request_id
            .parse()
            .expect("request id must be valid header value"),
    );
    response
}
