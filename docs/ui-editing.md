...
Part and assembly rotation is unrestricted: drag through the poles or turn the model upside down. Each drag increment rotates relative to the current screen axes, retaining any accumulated roll. Part view persistence includes roll; older saved views open with zero roll. Selecting a sketch plane still aligns the part to that plane.
