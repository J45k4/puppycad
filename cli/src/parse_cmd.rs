...
use puppycad_core::{
    build_model_state, build_render_state, codegen, eval::Evaluator, parser::parse_pcad,
};
...
impl RenderStateApp {
    ...
    fn load_render_state(
        ...
        let fill_position = target + (view_dir * (light_distance * 0.3))
...
impl pge::App<RenderEvent> for RenderStateApp {
    ...
    fn on_event(&mut self, event: RenderEvent, state: &mut pge::State) {
        match event {
            RenderEvent::LoadRenderState(render_state) => {
    ...
    match parse_pcad(&source) {
        Ok(ast) => {
            ...
            match output {
                ...
                ParseOutput::FeatureScript => match codegen::compile_to_three_json(&ast) {
                    Ok(json) => {
                        println!("{json}");
                    }
                    Err(err) => {
                ...
                ParseOutput::ModelState => {
                    match build_model_state(&puppycad_core::FeatureGraph::new(&ast)) {
                        Ok(state) => {
                            println!("{state:#?}");
                        }
                        Err(err) => {
...
pub fn run_render(args: RenderArgs) -> ExitCode {
    ...
    let screenshot_path =
        resolve_screenshot_path(args.output.as_deref(), args.output_dir.as_deref());
    ...
    let iterations = args
        .iterations
        .or(if should_run_headless { Some(1) } else { None });
...
"#;
    ...
    fn parses_three_component_camera_and_look_at_args() {
        ...
        assert_eq!(parse_vec3_arg(Some(&camera)), Some([0.0, 3.5, -12.0]));
